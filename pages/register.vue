...
const validateField = (field: string) => {
  ...
  if (field === 'email' && !/.+@.+\..+/.test(value)) {
    fieldErrors.value[field] = 'Email must be valid'
    return
  }
  ...
  if (field === 'confirmPassword' && value !== form.value.password) {
    fieldErrors.value[field] = 'Passwords must match'
    return
  }
...
async function handleRegister() {
  ...
  loading.value = true
  error.value = null
  fieldErrors.value = { username: '', email: '', password: '', confirmPassword: '' }
  ...
  try {
    ...
    error.value = handleApiError(err, 'Registration failed')
